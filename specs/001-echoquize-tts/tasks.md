...
- [X] T036 Run the full `specs/001-echoquize-tts/quickstart.md` validation (US1–US5) and tick items in `specs/001-echoquize-tts/checklists/readiness.md` — **closed 2026-06-10:** code-level slices re-verified PASS; live GUI/billed-API slices accepted as deferred.
  - **Code-level re-verification (2026-06-10, $0 spent):** US5 factory (local→`LocalStorage`, bogus→`ValueError`, s3→clear `NotImplementedError`); US3 `write_tags('x.pcm','pcm',{})`→`TagsNotSupportedError`; US1 DB layer `init_db()`+`list_generations()` returns the real 2026-06-08 mp3 row (541 KB file on disk — proof a live generation succeeded at least once); config fail-fast `raise ValueError` confirmed at `config.py:37-39`.
  - **Validated 2026-06-09 (no live API):** US4 container fail-fast w/o `OPENAI_API_KEY`, `docker compose up`→HTTP 200, `down` (no `-v`) preserves `./data/echoquize.db`.
  - **Operator-only remainder (deferred, accepted):** US1 single generate via UI, US2 batch+zip+bulk-cleanup, US3 mp3 tag→VLC, and US4 + optional-auth login require a live OpenAI key (billed) + browser/VLC — cannot be run by the agent without spending billed credits or driving a GUI. `readiness.md` items intentionally NOT ticked — they grade spec *wording*, not code.
